...
export function NewDaVinciWindow({ phase, onRenderComplete }: { phase: number, onRenderComplete?: () => void }) {
    ...
    >
      ...
      <div className="flex-1 flex gap-1 p-1 bg-black">
         ...
         <div className="w-[20%] bg-[#1A1A1A] rounded flex flex-col">
            ...
            <div className="flex-1 p-2 flex flex-wrap gap-2 content-start">
                <div className="w-[45%] aspect-video bg-indigo-900 rounded border border-gray-700"></div>
                {phase >= 3 && (
                    <>
                        <div className="w-[45%] aspect-video bg-blue-900 rounded border border-gray-700"></div>
                        <div className="w-[45%] aspect-video bg-purple-900 rounded border border-gray-700"></div>
                        <div className="w-[45%] aspect-video bg-pink-900 rounded border border-gray-700"></div>
                    </>
                )}
            </div>
         </div>
         ...
         <div className="flex-1 flex gap-1 relative">
             ...
             <AnimatePresence>
                {phase === 4 && (
                    ...
                    >
                        <div className="bg-[#1A1A1A] p-6 rounded-lg border border-gray-700 flex flex-col gap-4 w-96 shadow-2xl">
                            <span className="text-white font-bold text-lg">Rendering Video...</span>
                            <div className="w-full h-3 bg-black rounded-full overflow-hidden border border-gray-800">
                                <div className="h-full bg-blue-600" style={{ width: `${renderProgress}%` }}></div>
                            </div>
                            <span className="text-gray-400 text-sm text-right">{renderProgress}% Complete</span>
                        </div>
                    </motion.div>
                )}
             </AnimatePresence>
             ...
             <div className="flex-1 bg-[#1A1A1A] rounded flex flex-col">
                 ...
                 <div className="flex-1 p-2 flex items-center justify-center">
                     <div className="w-full aspect-video bg-black flex items-center justify-center border border-gray-800 relative">
                         {phase >= 3 && <div className="absolute inset-0 opacity-40 bg-gradient-to-tr from-blue-900 to-indigo-900"></div>}
                     </div>
                 </div>
             ...
             <div className="flex-1 bg-[#1A1A1A] rounded flex flex-col">
                 ...
                 <div className="flex-1 p-2 flex items-center justify-center">
                     <div className="w-full aspect-video bg-black border border-gray-800 relative overflow-hidden flex items-center justify-center">
                         ...
                         <motion.div 
                           className={`absolute inset-0 ${phase >= 2 ? 'bg-gradient-to-br from-blue-900/60 to-purple-900/60' : 'bg-gradient-to-br from-[#111] to-[#000]'} mix-blend-screen`}
                           animate={phase >= 2 ? { filter: ['hue-rotate(0deg)', 'hue-rotate(20deg)', 'hue-rotate(0deg)'] } : {}}
      ...
      <div className="flex h-[150px] md:h-[250px] bg-[#1A1A1A] border-t border-black flex-col">
         <div className="h-8 bg-[#242424] flex items-center px-4 justify-between border-b border-black">
             ...
             <div className="text-[10px] text-red-500 font-mono">01:00:03:140</div>
         </div>
         <div className="flex-1 flex overflow-hidden relative p-1">
             ...
             <div className="flex-1 relative bg-[#111]">
                 {/* Playhead */}
                 <div className="absolute top-0 bottom-0 w-px bg-red-500 z-20 pointer-events-none" style={{ left: '25%' }}>
                     <div className="absolute top-0 -left-1.5 w-3 h-3 bg-red-500 rounded-sm"></div>
                 </div>
                 
                 {/* V2 Track (Blue bg phase 2) */}
                 {phase >= 2 && (
                     <motion.div 
                        initial={{ width: 0, opacity: 0 }}
                        animate={{ width: "80%", opacity: 1 }}
                        className="absolute top-1 left-4 h-12 bg-blue-600/60 rounded border border-white/40 text-[10px] text-white p-1 overflow-hidden"
                     >
                         Solid_Color_Blue
                     </motion.div>
                 )}
                 ...
                 {/* V1 Track (Clips) */}
                 <div className="absolute top-[60px] left-4 w-[30%] h-12 bg-indigo-600/80 rounded border border-white/20 text-[10px] text-white/50 p-1">Intro.mp4</div>
                 {phase >= 3 && (
                     <>
                        <motion.div 
                           initial={{ x: 20, opacity: 0 }}
                           animate={{ x: 0, opacity: 1 }}
                           className="absolute top-[60px] left-[35%] w-[25%] h-12 bg-purple-600/80 rounded border border-white/20 text-[10px] text-white/50 p-1 overflow-hidden"
                        >B-Roll_1.mp4</motion.div>
                        <motion.div 
                           initial={{ x: 20, opacity: 0, width: "35%" }}
                           animate={{ x: 0, opacity: 1, width: "20%" }}
                           transition={{ duration: 1, delay: 0.5 }}
                           className="absolute top-[60px] left-[61%] h-12 bg-pink-600/80 rounded border-r-4 border-r-yellow-500 border-l border-white/20 text-[10px] text-white/50 p-1 overflow-hidden"
                        >B-Roll_Trimmed.mp4</motion.div>
                     </>
                 )}
                 
                 {/* A1 Track */}
                 <div className="absolute top-[120px] left-4 w-[80%] h-12 bg-teal-700/60 rounded border border-white/20 text-[10px] text-white/50 p-1">Main_Audio.wav</div>
             </div>
         </div>
      </div>
    </motion.div>
